...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['type']) && $_POST['type'] == "delete_kategori") {
    $id = $_POST['id'];
    $stmt = $pdo->prepare("DELETE FROM jenis_kegiatan WHERE id = ?");
    $stmt->execute([$id]);
    echo '<script>alert("Kategori berhasil dihapus!");location.href="?url=kategori";</script>';
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['type']) && $_POST['type'] == "update_kategori") {
    $id = $_POST['id'];
    $nama = $_POST['nama_kategori'];
    $stmt = $pdo->prepare("UPDATE jenis_kegiatan SET nama = ? WHERE id = ?");
    $stmt->execute([$nama, $id]);
    echo '<script>alert("Kategori berhasil diupdate!");location.href="?url=kategori";</script>';
    exit;
}
            ...
            <table class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <!-- Tombol Edit -->
                            <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modal-edit-kategori-<?= $jk['id'] ?>">Edit</button>
                            <!-- Tombol Hapus -->
                            <form method="post" style="display:inline;">
                                <input type="hidden" name="id" value="<?= $jk['id'] ?>">
                                <input type="hidden" name="type" value="delete_kategori">
                    ...
                    <!-- Modal Edit Kategori -->
                    <div class="modal fade" id="modal-edit-kategori-<?= $jk['id'] ?>">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form method="post">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <label>Nama Kategori</label>
                                            <input type="text" name="nama_kategori" class="form-control" value="<?= htmlspecialchars($jk['nama']) ?>" required>
                                        </div>
                                        <input type="hidden" name="id" value="<?= $jk['id'] ?>">
                                        <input type="hidden" name="type" value="update_kategori">
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
                                        <button type="submit" class="btn btn-primary">Simpan</button>
